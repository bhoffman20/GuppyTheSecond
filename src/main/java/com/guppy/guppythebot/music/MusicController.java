	...
	private void addTrack(final Message message, final String identifier, final boolean now, final boolean next)
	{
		...
		manager.loadItemOrdered(this, identifier, new AudioLoadResultHandler()
		{
			...
			public void trackLoaded(AudioTrack track)
			{
				...
				message.getChannel()
						.sendMessage("Starting now: " + track.getInfo().title + " (length " + track.getDuration() + ")")
						.queue();
			...
			public void playlistLoaded(AudioPlaylist playlist)
			{
				...
				{
					message.getChannel()
							.sendMessage("Loaded playlist: " + playlist.getName() + " (" + tracks.size() + ")").queue();
				}
		...
		else
		{
			...
			else /* if (sb.length() <= 20000) */
			{
				try
				{
					...
					HttpResponse response = Unirest.post("https://hastebin.com/documents").body(sb.toString())
							.asString();
